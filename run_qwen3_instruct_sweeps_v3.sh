#!/usr/bin/env bash
# Qwen3 instruct steering sweeps v3
#
# Models: Qwen3-1.7B, Qwen3-8B, Qwen3-14B
# Note: These ARE the instruct models — Qwen3 does not have separate -Instruct
#       variants for these sizes; -Base suffix marks the raw pretrained model.
#
# Changes vs v2:
#   - Results saved to a dated folder: results/YYYY-MM-DD_HHMMSS/
#   - max_new_tokens=1024
#   - Steering vectors re-used from previous run (already exist)
#
# Usage:
#   bash run_qwen3_instruct_sweeps_v3.sh 2>&1 | tee run_qwen3_instruct_sweeps_v3.log
...
git add "$RESULTS_DIR/" .gitignore run_qwen3_instruct_sweeps_v3.sh \
        sweep_steering.py evaluate.py
git commit -m "Add Qwen3 instruct sweeps v3 — dated results folder ${RUN_TS}

- Models: Qwen3-1.7B, Qwen3-8B, Qwen3-14B (instruct, no separate -Instruct variant)
- max_new_tokens=1024, temp=0, lin_only filter, in-dist val set (9 situations)
- Single-layer: 5 layers x 7 alphas + 3 multilayer combos per model
- Results in: results/${RUN_TS}/
