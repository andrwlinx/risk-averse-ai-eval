#!/usr/bin/env bash
# Qwen3 instruct steering sweeps v5
#
# Models: Qwen3-1.7B, Qwen3-8B, Qwen3-14B
# Changes vs v4:
#   - VAL_CSV uses OOD val set (2026_01_29_new_val_set_probabilities_add_to_100.csv)
#
# Usage:
#   bash run_qwen3_instruct_sweeps_v5.sh 2>&1 | tee run_qwen3_instruct_sweeps_v5.log
...
echo "============================================================"
echo "Qwen3 Instruct Steering Experiments v5 — $(date)"
echo "Models: Qwen3-1.7B, Qwen3-8B, Qwen3-14B"
echo "Eval: lin_only filter, OOD val set, max_new_tokens=4096, temp=0"
...
git add "$RESULTS_DIR/" run_qwen3_instruct_sweeps_v5.sh \
        sweep_steering.py evaluate.py generate_steering_vector.py
git commit -m "$(cat <<'EOF'
Add Qwen3 steering sweeps v5 — OOD val set

- Models: Qwen3-1.7B, Qwen3-8B, Qwen3-14B
- max_new_tokens=4096, temp=0, lin_only filter, OOD val set
